function extract_audioLogger_data(input_folder,output_folder_str)
...
Nlg_folders={input_folder}; % each cell is a folder where Nlg voltage data files and event file are stored  

output_folders_for_each_Nlg_folder={output_folder_str}; % each cell is a folder where the outputs of the code (voltage and event data in MATLAB or Nlx formats) will be saved, corresponding to one of the folders in Nlg_folders

inactive_channels_for_each_Nlg_folder=cell(1,length(Nlg_folders)); % each cell is a vector containing numbers between 1 and the number of channels, indicating the disabled channels; enter an empty vector if all channels are active; each cell corresponds to one of the folders in Nlg_folders
reference_channel_for_each_Nlg_folder=cell(1,length(Nlg_folders)); % each cell is a number between 1 and the number of channels, indicating a reference channel whose AD counts (equivalently, voltages) will be subtracted from those of all other channels during the processing here; enter an empty vector to not subtract any reference channel here; each cell corresponds to one of the folders in Nlg_folders
...
global_or_local_clock_difference_estimation=2; % to estimate unreported clock differences, 1: fit a line using all reported clock differences in a recording interval where clocks are not synchronized; 2: interpolate between consecutive time points with reported clock differences
%%
...
% to using 512/15 us).
samples_per_channel_per_file=[]; % specify here the number of samples per channel per Nlg .DAT file; alternatively, leave as [] to automatically calculate this from the number of samples in the first file loaded and the number of channels, num_channels
...
Nlg_data_type='int16'; % the data type of the AD counts in the Nlg .DAT files
% For the Neurolog-16 that we use as of this writing (7/11/2016), the data
% type is 16-bit unsigned integer.
Nlg_unwritten_data_value=0; % specify here the default value of Nlg voltage data samples when they haven't been written with actual recorded data
...
%     clock_differences_usec(clock_differences_usec>0)= clock_differences_usec(clock_differences_usec>0) - (60*60*24)*1e6;
    ...
    if save_voltage_AD_count_files
        ...
        for Nlg_file_i=1:num_Nlg_files % for each Nlg .DAT file
            ...
            Nlg_file_name=[Nlg_file_name_letters '0' Nlg_file_number_string '.DAT']; % eg. "NEUR0003.DAT"
